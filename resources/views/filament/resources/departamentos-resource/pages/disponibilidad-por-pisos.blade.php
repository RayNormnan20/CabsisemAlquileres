                            ...
                            <div class="flex flex-wrap gap-3 flex-1">
                                @foreach($departamentos as $departamento)
                                    ...
                                    <div class="text-white rounded-lg p-4 min-w-[85px] text-center shadow-sm hover:shadow-lg transition-all duration-200 cursor-pointer transform hover:scale-105 relative group"
                                        ...
                                        <!-- Tooltip -->
                                        <div class="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-3 py-2 bg-white text-gray-800 text-xs rounded-lg shadow-lg border border-gray-200 opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-[99999] whitespace-nowrap">
                                            <div class="font-bold">Departamento Nro. {{ $departamento['numero_departamento'] }}</div>
                                            <div class="text-xs mt-1">
                                                <div class="text-left"><span class="font-bold">Tipo moneda:</span> {{ $departamento['tipo_moneda'] ?? 'Soles' }}</div>
                                                <div class="text-left"><span class="font-bold">Precio:</span> S/ {{ number_format($departamento['precio_alquiler'] ?? 0, 2) }}</div>
                                                <div class="mt-1">
                                                    <div class="text-left"><span class="font-bold">Nro Hab:</span> {{ $departamento['cuartos'] ?? '1' }}</div>
                                                    <div class="text-left"><span class="font-bold">Nro Baños:</span> {{ $departamento['banos'] ?? '1' }}</div>
                                                    <div class="text-left"><span class="font-bold">Área TOTAL:</span> {{ $departamento['metros_cuadrados'] ?? '0' }}m2</div>
                                                </div>
                                                <div class="mt-1">
                                                    <div class="font-bold text-orange-600">INMUEBLES EN OPERACIÓN</div>
                                                    <div class="text-left"><span class="font-bold">Departamento:</span> {{ $departamento['numero_departamento'] }}</div>
                                                    <div class="text-left"><span class="font-bold">Precio:</span> S/ {{ number_format($departamento['precio_alquiler'] ?? 0, 2) }}</div>
                                                    <div class="mt-1">
                                                        <div class="text-left"><span class="font-bold">Estado:</span> {{ $estado }}</div>
                                                        @if(isset($departamento['alquileres'][0]['inquilino']))
                                                            @php
                                                                $inquilino = $departamento['alquileres'][0]['inquilino'];
                                                                $nombreCompleto = ($inquilino['nombre'] ?? '') . ' ' . ($inquilino['apellido'] ?? '');
                                                            @endphp
                                                            <div class="text-left"><span class="font-bold">Cliente:</span> {{ trim($nombreCompleto) }}</div>
                                                        @endif
                                                    </div>
                                                </div>
                                            </div>
