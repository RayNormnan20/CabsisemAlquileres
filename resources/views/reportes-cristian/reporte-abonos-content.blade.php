...
foreach ($datosAbonos as $abono) {
    foreach ($abono->conceptosabonos as $conceptoAbono) {
        ...
        // Si el registro es una devolución, reetiquetar Yape/Efectivo como Devolucion (...)
        if ($abono->es_devolucion) {
            if ($conceptoOriginal === 'Yape') {
                $conceptoOriginal = 'Devolucion (Yape)';
            } elseif ($conceptoOriginal === 'Efectivo') {
                $conceptoOriginal = 'Devolucion (Efectivo)';
            }
        }
