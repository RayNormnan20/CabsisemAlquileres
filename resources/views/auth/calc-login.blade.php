...
<head>
    ...
    <style>
        ...
        * { box-sizing:border-box; }
        ...
        .grid { display:grid; grid-template-columns:repeat(4,1fr); gap:1px; background:#2e3944; }
...
<script>
    (function(){
        ...
        let step = 1; // 1: celular, 2: contraseña
        let value = '';
        let exp = '';
        ...
        const hadServerError = {!! json_encode($errors->has('login')) !!};
        if (hadServerError) {
            step = 1; value = ''; exp = ''; setError(''); render();
        }
        ...
        document.querySelectorAll('[data-action="dot"]').forEach(btn => {
            btn.addEventListener('click', () => {
                if(step===1){
                    if (!value.endsWith('.')) { value += '.'; render(); setError(); }
                } else {
                    if (!exp.endsWith('.')) { exp += '.'; render(); setError(); }
                }
            });
        });
        ...
        if(btnClear) btnClear.addEventListener('click', () => { if(step===1){ value=''; } else { exp=''; } render(); setError(); });
        if(btnBack) btnBack.addEventListener('click', () => { if(step===1){ value = value.slice(0,-1); } else { exp = exp.slice(0,-1); } render(); setError(); });

        // Operadores en ambas fases (calculadora)
        const addOp = (op) => {
            if (step === 1) {
                if (value.length === 0) return;
                if (/[+\-*/]$/.test(value)) { value = value.slice(0,-1) + op; }
                else { value += op; }
            } else {
                if (exp.length === 0) return;
                if (/[+\-*/]$/.test(exp)) { exp = exp.slice(0,-1) + op; }
                else { exp += op; }
            }
            render(); setError();
        };
        ...
        if(btnEqual) btnEqual.addEventListener('click', () => {
            if(step === 1){
                ...
                if (/[+\-*/]/.test(value)) {
                    ...
                    loginInput.value = value; value=''; exp=''; step=2; render(); setError();
                }
            } else {
                ...
                if (/[+\-*/]/.test(exp)) {
                    ...
                    passwordInput.value = exp; form.submit();
                }
            }
        });
    })();
